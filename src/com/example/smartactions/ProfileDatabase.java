...
public class ProfileDatabase extends SQLiteOpenHelper {
	...
	private static final String AUTO = "auto_reply";
	...
	public void onCreate(SQLiteDatabase db) {
		String CREATE_CONTACTS_DATA = "CREATE TABLE Profile "  + "("+ NAME+","+RINGVOL+","+MODE+","+ACTIVE+","+AUTO+","+BTOOTH+","+WIFI+","+BRIGHT+")";
		...
		values.put(ACTIVE,"0");
		values.put(AUTO,"0");
		values.put(BTOOTH, "NC");
		values.put(WIFI, "NC");
		values.put(BRIGHT, "200");
		...
		values.put(BRIGHT, "200");
		db.insert("Profile", null, values);
		values.put(NAME, "Driving"); // Contact Name
		values.put(RINGVOL, "0");
		values.put(MODE, "S");
		values.put(ACTIVE,"0");
		values.put(AUTO,"1");
		...
		values.put(BRIGHT, "200");
		db.insert("Profile", null, values);
		values.put(NAME, "Meeting"); // Contact Name
		values.put(RINGVOL, "0");
		values.put(MODE, "S");
		values.put(ACTIVE,"0");
		values.put(AUTO,"1");
	...
	public void addContact(Contact contact) {
		...
		values.put(AUTO, contact.auto);
		values.put(BTOOTH, contact.btooth);
		values.put(WIFI, contact.wifi);
		values.put(BRIGHT, contact.bright);
		db.insert("Profile", null, values);
		db.close(); // Closing database connection
	}
	
	Contact getActCont(String name){

		SQLiteDatabase db = this.getWritableDatabase();
		
		Cursor cursor = db.rawQuery("Select * from "+ "Profile" + " where "+ACTIVE+" = ? ",new String[] {name});
	...
	Contact getCont(String act){

		SQLiteDatabase db = this.getWritableDatabase();
		
		Cursor cursor = db.rawQuery("Select * from "+ "Profile" + " where profilename = ? ",new String[] {act});
		if (cursor.getCount()!=0)
		{
			cursor.moveToFirst();
			Contact contact = new Contact(
	...
	public ArrayList<Contact> getAllProfiles() {
		...
		if (cursor.getCount()!=0) {
			...
			do {
				...
				user.auto=cursor.getString(4);
				user.btooth=cursor.getString(5);
				user.wifi=cursor.getString(6);
				user.bright=Integer.parseInt(cursor.getString(7));
				userList.add(user);
